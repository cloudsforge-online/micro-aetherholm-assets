...
async function readManifest(): Promise<Manifest> {
  ...
  const parsed = JSON.parse(await readFile(MANIFEST, 'utf8')) as { assets?: ManifestEntry[] }
  const out: Manifest = {}
...
async function generateOne(
  ...
  const prompt = promptFor(planned)
  ...
  for (let go = 0; go <= MAX_TRANSIENT; go += 1) {
    try {
      ...
      const dir = join(ASSETS, directory!)
...
async function main(): Promise<void> {
  ...
  if (!selection.deriveOnly) {
    ...
    let work = plannedAssets().filter((asset) => {
      ...
      const requested = requestSizeFor({ width: asset.width, height: asset.height })
      const onGrid = requested.width === asset.width && requested.height === asset.height
      const recorded = onGrid ? asset.key : `${asset.key}-source`
      const size = onGrid
        ? `${asset.width}x${asset.height}`
        : `${requested.width}x${requested.height}`
      return manifest[keyOf(recorded, size)] === undefined
    })
    ...
    const worker = async (): Promise<void> => {
      for (;;) {
        ...
        const size = onGrid
          ? `${asset.width}x${asset.height}`
          : `${requested.width}x${requested.height}`
        const previous = manifest[keyOf(recorded, size)]
        ...
        try {
          ...
          try {
            entry = await generateOne(backend, asset, previousRetries)
          } catch (err) {
            ...
            entry = await generateOne(backend, asset, previousRetries + 1)
          }
  ...
  for (const entry of await derive()) manifest[keyOf(entry.asset, entry.declaredSize)] = entry
  await writeManifest(manifest)
